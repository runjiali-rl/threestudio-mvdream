...
@threestudio.register("partdream-guidance-legacy")
class PartDreamDiffusionGuidanceLegacy(BaseObject):
    @dataclass
    class Config(BaseObject.Config):
        model_name: str = (
            "sd-v2.1-base-4view"  # check mvdream.model_zoo.PRETRAINED_MODELS
    ...
    def __call__(
        ...
        text_embeddings=None,
        input_is_latent=False,
        is_global_step: bool = False,
        gaussian_var_div: float = 4.0,
        **kwargs,
    ):
        batch_size = rgb.shape[0]
        camera = c2w
        ...
        def gaussian_filter(H, W, std_dev):
            ...
            gauss = torch.exp(-((x - x_center) ** 2 + (y - y_center) ** 2) / (2 * std_dev ** 2+1e-9))
        ...
        def hook(grad):
            gaussian_heat_map = gaussian_filter(rgb_BCHW.shape[2],
                                                rgb_BCHW.shape[3],
                                                rgb_BCHW.shape[2] / gaussian_var_div).to(rgb_BCHW.device)
            grad = grad * gaussian_heat_map
            del gaussian_heat_map
            return grad
        # if the weight_filters is None and it is not a global step, apply the gaussian filter on the gradient
        ...
        if self.cfg.recon_loss:
            ...
            grad = torch.autograd.grad(loss, latents, retain_graph=True)[0]
            image_grad = torch.autograd.grad(loss, rgb, retain_graph=True)[0].detach().clone()
    ...
    def __call__(
        ...
        **kwargs,
        ...
        def gaussian_filter(H, W, std_dev):
            ...
            # Compute the Gaussian function
            gauss = torch.exp(-((x - x_center) ** 2 + (y - y_center) ** 2) / (2 * std_dev ** 2))
        ...
        def hook(grad):
            ...
            return grad
        # if the weight_filters is None and it is not a global step, apply the gaussian filter on the gradient
        if weight_filters is None and not is_global_step:
            handle = rgb_BCHW.register_hook(hook)
        else:
            handle = None
            ...
            if self.cfg.recon_std_rescale > 0:
                ...
                if is_global_step:
                    if weight_filters is not None:
                        ...
                        # decoded_latents_recon = self.model.decode_first_stage(latents_recon)
                        # if weight_filters.shape[-1] != decoded_latents_recon.shape[-1]:
                        #     weight_filters = F.interpolate(
                        #         weight_filters,
                        #         size=(decoded_latents_recon.shape[-2], decoded_latents_recon.shape[-1]),
                        #         mode="bilinear",
                        #         align_corners=False,
                        #     )

                        # latents_recon = decoded_latents_recon * weight_filters
                        # latents = pred_rgb * weight_filters

                        

                        # only learn the gaussian filter if the filter is provieded
                        handle = rgb_BCHW.register_hook(lambda grad: grad * 0)
                        # handle = weight_filters.register_hook(lambda grad: grad * -1)
...
@threestudio.register("mvdream-multiview-geometry-diffusion-guidance")
class MultiviewGeometryDiffusionGuidance(BaseObject):
    @dataclass
    class Config(BaseObject.Config):
        model_name: str = (
            "sd-v2.1-base-4view"  # check mvdream.model_zoo.PRETRAINED_MODELS
